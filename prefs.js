...
const DISPLAY_CONFIG_IFACE = '<node>' +
    ...
    '  </interface>' +
    '</node>';
const DisplayConfigProxy = Gio.DBusProxy.makeProxyWrapper(DISPLAY_CONFIG_IFACE);
...
class Rule extends GObject.Object {
    static [GObject.properties] = {
        ...
        'workspace': GObject.ParamSpec.uint(
            ...
            GObject.ParamFlags.READWRITE,
            1, WORKSPACE_MAX, 1),
        'monitor': GObject.ParamSpec.string(
            'monitor', null, null,
            GObject.ParamFlags.READWRITE, ''),
    };
...
class MonitorEntry extends GObject.Object {
    static [GObject.properties] = {
...
class MonitorsModel extends GObject.Object {
    static {
        GObject.registerClass(this);
    }
    ...
    constructor() {
        ...
        this.#store = new Gio.ListStore({item_type: MonitorEntry});

        this.#proxy = null;
        try {
            this.#proxy = new DisplayConfigProxy(Gio.DBus.session,
                'org.gnome.Mutter.DisplayConfig',
                '/org/gnome/Mutter/DisplayConfig');
        } catch (err) {
            log(`Failed to connect to DisplayConfig: ${err}`);
        }

        this.#signalId = 0;
        if (this.#proxy) {
            this.#signalId = this.#proxy.connectSignal('MonitorsChanged',
                () => this.refresh());
        }
    ...
    refresh() {
        ...
        this.#store.append(new MonitorEntry({connector: '', label: _('Default')}));

        if (!this.#proxy)
            return;

        try {
            const [, monitors] = this.#proxy.GetCurrentStateSync();
            monitors?.forEach(monitor => {
                ...
                const label = parts.length > 0
                    ...
                    : connector;
                this.#store.append(new MonitorEntry({connector, label}));
            });
        } catch (err) {
            log(`Failed to list displays: ${err}`);
        }
    }
...
class RulesList extends GObject.Object {
    ...
    changeMonitor(id, monitor) {
        const pos = this.#rules.findIndex(r => r.appInfo.get_id() === id);
        if (pos < 0)
            return;

        this.#rules[pos].set({monitor});
        this.#saveRules();
    }
    ...
    #saveRules() {
        ...
        this.#settings.set_strv(SETTINGS_KEY,
            this.#rules.map(r => {
                const base = `${r.app_info.get_id()}:${r.workspace}`;
                return r.monitor ? `${base}:${r.monitor}` : base;
            }));
    ...
    #sync() {
        ...
        for (const stringRule of this.#settings.get_strv(SETTINGS_KEY)) {
            const [id, workspace, monitor = ''] = stringRule.split(':');
            const appInfo = GioUnix.DesktopAppInfo.new(id);
            if (appInfo)
                this.#rules.push(new Rule({
                    appInfo,
                    workspace: Number.parseInt(workspace, 10),
                    monitor,
                }));
            else
                log(`Invalid ID ${id}`);
...
class AutoMoveSettingsWidget extends Adw.PreferencesGroup {
    static {
        ...
        this.install_action('rules.change-monitor', '(ss)',
            (self, name, param) => self._rules.changeMonitor(...param.deepUnpack()));
    }

    constructor(settings) {
        super({
            title: _('Workspace and Display Rules'),
        });

        this._settings = settings;
        this._rules = new RulesList(this._settings);
        this._monitors = new MonitorsModel();
        ...
        this._list.bind_model(listModel, item => {
            return item instanceof NewItem
                ? new NewRuleRow()
                : new RuleRow(item, this._monitors);
        });
    }
...
class DisplaySelector extends Gtk.DropDown {
    ...
    constructor(model) {
        super({
            model,
            expression: new Gtk.PropertyExpression(MonitorEntry, null, 'label'),
            valign: Gtk.Align.CENTER,
            tooltip_text: _('Display'),
        });

        this.connect('notify::selected', () => {
            const item = this.selected_item ?? null;
            const connector = item?.connector ?? '';
            if (this.#connector === connector)
                return;

            this.#connector = connector;
            this.emit('changed', connector);
        });

        model.connect('items-changed', () => this.#syncSelection());
        this.#syncSelection();
    }
    ...
    #syncSelection() {
        ...
        for (let i = 0; i < model.get_n_items(); i++) {
            if (model.get_item(i)?.connector === this.#connector) {
                if (this.selected !== i)
                    this.selected = i;
                return;
            }
...
class RuleRow extends Adw.ActionRow {
    ...
    constructor(rule, monitors) {
        ...
        const displaySelector = new DisplaySelector(monitors.store);
        displaySelector.connector = rule.monitor;
        this.add_suffix(displaySelector);

        rule.connect('notify::monitor', () => {
            displaySelector.connector = rule.monitor;
        });
        displaySelector.connect('changed', (selector, connector) => {
            this.activate_action('rules.change-monitor',
                new GLib.Variant('(ss)', [id, connector]));
        });
